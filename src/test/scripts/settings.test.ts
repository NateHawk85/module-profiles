...
import { v9 } from '../../main/scripts/version-strategies';
...
describe('getCurrentModuleConfiguration', () =>
{
	describe('v9', () =>
	{
		beforeEach(() =>
		{
			jest.spyOn(Settings, 'getFoundryVersionStrategy').mockReturnValue(v9);
		});

		test.each(Constants.CoreModulesConfigurationToCorrespondingModuleInfosPairs)
			('WHEN game.modules returns a map of modules THEN maps them to an array of ModuleInfo objects: %o, %o',
				(coreConfiguration, expectedProfile) =>
				{
		...
		test.each([
			...
			('WHEN game.modules returns a map of modules unsorted by title THEN sorts them by title: %o, %o',
				(coreConfiguration, expectedModuleInfos) =>
				{
					// @ts-ignore - Mocking for Foundry
					game.modules = coreConfiguration;
...
describe('setCoreModuleConfiguration', () =>
{
	...
	test.each(Constants.CoreSettingsModuleConfigurations)
		(
			...
			async ({ moduleProfilesEditProfileName, ...expectedConfiguration }) =>
			{
				MappingUtils.mapToModuleKeyIsActiveRecord.mockReturnValue(expectedConfiguration as Record<string, boolean>);
	...
	test.each(Constants.CoreSettingsModuleConfigurations)
		('WHEN called with info THEN returns what game.settings.set returns: %o',
			async ({ moduleProfilesEditProfileName, ...expectedConfiguration }) =>
			{
				MappingUtils.mapToModuleKeyIsActiveRecord.mockReturnValue(expectedConfiguration as Record<string, boolean>);
				when(game.settings.set)
					.calledWith('core', 'moduleConfiguration', expectedConfiguration)
					.mockReturnValue(Promise.resolve(expectedConfiguration));
	...
	test(
		'WHEN core module configuration returns more values than module infos entered THEN calls game.settings.set with merged values from the two',
		() =>
		{
			...
			when(game.settings.get)
				.calledWith('core', 'moduleConfiguration')
				.mockReturnValue(coreModuleConfigurationResponse);
	...
	test(
		'WHEN core module configuration returns less values than module infos entered THEN calls game.settings.set with merged values from the two',
		() =>
		{
			const coreModuleConfigurationResponse: Record<string, boolean> = {
				'some-other-module': true,
	...
	test(
		'WHEN passed modules have different values than core module configuration THEN inputs overwrite core module configuration values',
		() =>
		{
			const coreModuleConfigurationResponse: Record<string, boolean> = {
...
describe('createProfile', () =>
{
	...
	{
		const functionCall = async () => Settings.createProfile({
			// @ts-ignore
			name: undefined,
			description: '',
	...
	test('WHEN profile name is empty string THEN throws Error and calls ui.notifications.error', async () =>
	{
		...
		expect(ui.notifications.error)
			.toHaveBeenCalledWith('Unable to create module profile. Profile name must not be empty.');
	...
	test('WHEN profile name is defined and modules are undefined THEN throws Error and calls ui.notifications.error',
		async () =>
		{
			...
			await expect(functionCall).rejects.toThrow(Error);
			expect(ui.notifications.error)
				.toHaveBeenCalledWith('Unable to create module profile. Please refresh the page and try again.');
			await expect(functionCall)
				.rejects
				.toThrow('Unable to create module profile. Please refresh the page and try again.');
		});
	...
	test.each(Constants.ModuleProfileNames)
		('WHEN profile already exists THEN throws Error and calls ui.notifications.error: %s', async (value) =>
		{
			...
			await expect(functionCall).rejects.toThrow(Error);
			expect(ui.notifications.error)
				.toHaveBeenCalledWith(`Unable to create module profile. Profile "${value}" already exists!`);
			await expect(functionCall)
				.rejects
				.toThrow(`Unable to create module profile. Profile "${value}" already exists!`);
		});

	test.each(Constants.NameModuleProfilePairs)
		('WHEN no profiles exist THEN calls SettingsUtils.setProfiles to save profile: %s, %o',
			async (profileName, testProfile) =>
			{
				jest.spyOn(Settings, 'getProfileByName').mockReturnValue(undefined);
				jest.spyOn(Settings, 'getAllProfiles').mockReturnValue([]);

				await Settings.createProfile({
					name: profileName,
					description: '',
	...
	test.each(Constants.NameModuleProfilePairs)
		('WHEN profile name does not exist THEN calls SettingsUtils.setProfiles to save profile: %s, %o',
			async (profileName, testProfile) =>
			{
				jest.spyOn(Settings, 'getProfileByName').mockReturnValue(undefined);
				const existingProfile = { name: 'A Unique Profile', description: '', modules: [] };
	...
	test.each(Constants.ModuleProfileNames)
		...
		{
			jest.spyOn(Settings, 'getAllProfiles')
				.mockReturnValue([{ name: 'Some random name', description: '', modules: [] }]);
	...
	test.each(Constants.SavedModuleProfilesArrays)
		('WHEN no matching profile exists THEN returns what SettingsUtils.setProfiles returns: %s', async (value) =>
		{
			...
			const response = await Settings.createProfile({
...
describe('activateProfile', () =>
{
	...
	test.each(Constants.ModuleProfileNames)
		('WHEN profile does not exist THEN throws Error and calls ui.notifications.error: %s', async (value) =>
		{
			...
			await expect(functionCall).rejects.toThrow(Error);
			expect(ui.notifications.error)
				.toHaveBeenCalledWith(`Unable to activate module profile. Profile "${value}" does not exist!`);
			await expect(functionCall)
				.rejects
				.toThrow(`Unable to activate module profile. Profile "${value}" does not exist!`);
		});

	test.each(Constants.NameModuleProfilePairs)
		('WHEN profile exists THEN calls SettingsUtils.setActiveProfileName to save active profile name: %s, %o',
...
describe('saveChangesToProfile', () =>
{
	test.each(Constants.SavedModuleProfilesArrays)
		('WHEN no profiles exist with name THEN throws Error and calls ui.notifications.error: %o, %s', async (profiles) =>
		{
			...
			const functionCall = async () => Settings.saveChangesToProfile('Some Name Does Not Exist',
				{ modules: DEFAULT_PROFILE.modules });

			await expect(functionCall).rejects.toThrow(Error);
			expect(ui.notifications.error)
				.toHaveBeenCalledWith(
					`Unable to save module profile changes. Profile "Some Name Does Not Exist" does not exist!`);
			await expect(functionCall)
				.rejects
				.toThrow(`Unable to save module profile changes. Profile "Some Name Does Not Exist" does not exist!`);
		});
	...
	test('WHEN profile exists with name THEN fires "moduleProfilesUpdated" hook event', async () =>
	{
		...
		await Settings.saveChangesToProfile(DEFAULT_PROFILE_NAME, { modules: DEFAULT_PROFILE.modules });
	...
	test.each(Constants.ModuleProfileNames)
		('WHEN profile exists with name THEN calls ui.notifications.info: %s', async (value) =>
		{
			...
			await Settings.saveChangesToProfile(value, { modules: DEFAULT_PROFILE.modules });
	...
	test.each(Constants.ModuleProfilesAsArray)
		('WHEN profile exists with name THEN returns what SettingsUtils.setProfiles returns: %s', async (value) =>
		{
			...
			const actual = await Settings.saveChangesToProfile(DEFAULT_PROFILE_NAME, { modules: DEFAULT_PROFILE.modules });
...
describe('getActiveProfile', () =>
{
	...
	test('WHEN no profile exists with active profile name THEN throws Error and calls ui.notifications.error', () =>
	{
		...
		expect(ui.notifications.error)
			.toHaveBeenCalledWith('Unable to load active profile. Please refresh the Foundry page.');
...
describe('getProfileByName', () =>
{
	...
	test.each(Constants.SavedModuleProfilesArrays)
		('WHEN Settings.getAllProfiles returns an array with no matching profile names THEN returns undefined: %s',
			(value) =>
			{
				jest.spyOn(Settings, 'getAllProfiles').mockReturnValue(value);
	...
	test.each(Constants.NameModuleProfilePairs)
		('WHEN Settings.getAllProfiles returns one profile and name matches THEN returns the matching profile: %s, %o',
			(profileName, moduleProfile) =>
			{
...
describe('importProfiles', () =>
{
		...
		test.each([
			...
			(
				...
				async (value) =>
				{
					...
					await expect(functionCall).rejects.toThrow(Error);
					expect(ui.notifications.error)
						.toHaveBeenCalledWith('Unable to import profiles. Please re-export and try again.');
	...
	{
		test.each(Constants.ModuleProfilesAsArray)
			('WHEN JSON.parse returns a single module profile THEN calls Settings.createProfile once for that profile: %s',
				async (value) =>
				{
					// @ts-ignore
					JSON.parse.mockReturnValue(value);
		...
		test.each(Constants.ModuleProfilesAsArray)
			('WHEN JSON.parse returns a single module profile THEN returns what Settings.getAllProfiles returns: %s',
				async (value) =>
				{
					// @ts-ignore
					JSON.parse.mockReturnValue(DEFAULT_PROFILE);
	...
	describe('Array of profiles', () =>
	{
		test.each(Constants.ModuleProfilesAsArray)
			(
				'WHEN JSON.parse returns a single module profile as array THEN calls Settings.createProfile once for that profile: %s',
				async (value) =>
				{
					// @ts-ignore
					JSON.parse.mockReturnValue([value]);
		...
		test.each([
			...
			[
				[
					Constants.TestModuleProfiles.MultipleAllEnabled, DEFAULT_PROFILE,
					Constants.TestModuleProfiles.OnlyModuleProfiles,
				],
			],
			...
			('WHEN JSON.parse returns multiple module profiles THEN calls Settings.createProfile one per profile: %o',
				async (value) =>
				{
					// @ts-ignore
					JSON.parse.mockReturnValue(value);
		...
		test.each([
			...
			('WHEN JSON.parse returns multiple module profiles THEN returns what Settings.getAllProfiles returns: %o',
				async (value) =>
				{
					// @ts-ignore
					JSON.parse.mockReturnValue(DEFAULT_PROFILE);
...
describe('exportAllProfiles', () =>
{
	...
	test(
		'WHEN Settings.getAllProfiles returns profiles THEN returns what Settings.getAllProfiles returns as JSON string with alternate parameters',
		() =>
		{
			const profile: ModuleProfile = {
				'name': 'A Profile Name',
...
describe('exportProfileByName', () =>
{
	...
	test(
		'WHEN Settings.getProfileByName returns a profile THEN returns what Settings.getProfileByName returns as JSON string',
		() =>
		{
			const profile: ModuleProfile = {
				'name': 'A Profile Name',
	...
	test(
		'WHEN Settings.getProfileByName returns a profile THEN returns what Settings.getProfileByName returns as JSON string with alternate parameters',
		() =>
		{
			const profile: ModuleProfile = {
				'name': 'A Different Profile Name',
...
describe('deleteProfile', () =>
{
	test('WHEN no profile exists with name THEN throws Error and calls ui.notifications.error', async () =>
	{
		...
		await expect(functionCall).rejects.toThrow(Error);
		expect(ui.notifications.error)
			.toHaveBeenCalledWith(`Unable to delete module profile. Profile "${DEFAULT_PROFILE_NAME}" does not exist!`);
		await expect(functionCall)
			.rejects
			.toThrow(`Unable to delete module profile. Profile "${DEFAULT_PROFILE_NAME}" does not exist!`);
	});
	...
	test.each([
		...
		[
			...
			[
				DEFAULT_PROFILE, Constants.TestModuleProfiles.OnlyModuleProfilesAndTidyUI,
				Constants.TestModuleProfiles.MultipleAllDisabled,
			],
		],
		[
	...
	test(
		'WHEN deleted profile was not the active profile THEN does not set the active profile name to the first profile available',
		async () =>
		{
			jest.spyOn(Settings, 'getProfileByName').mockReturnValue(DEFAULT_PROFILE);
			jest.spyOn(Settings, 'getAllProfiles')
				.mockReturnValue([DEFAULT_PROFILE, Constants.TestModuleProfiles.OnlyModuleProfiles]);
	...
	test.each(Constants.SavedModuleProfilesArrays)
		('WHEN deleted profile was the active profile THEN sets the active profile name to the first profile available',
			async (value) =>
			{
				jest.spyOn(Settings, 'getProfileByName').mockReturnValue(DEFAULT_PROFILE);
				jest.spyOn(Settings, 'getAllProfiles')
					.mockReturnValue([{ name: 'A Profile', description: '', modules: [] }, ...value]);
	...
	test.each(Constants.ModuleProfilesAsArray)
		('WHEN profile exists with name THEN returns what SettingsUtils.setProfiles returns: %s', async (value) =>
		{
			jest.spyOn(Settings, 'getProfileByName').mockReturnValue({ name: 'A Profile', description: '', modules: [] });
			jest.spyOn(Settings, 'getAllProfiles')
				.mockReturnValue([value, { name: 'A Profile', description: '', modules: [] }]);
			SettingsUtils.setProfiles.mockReturnValue(Promise.resolve([value]));
...
describe('getFoundryVersionStrategy', () =>
{
	test.each(['9.265', '9.280'])
		('WHEN is v9 THEN returns v9', (value) =>
		{
			...
			const response = Settings.getFoundryVersionStrategy();

			expect(response.version).toStrictEqual(9);
		});
	...
	test.each(['10.282', '10.285'])
		('WHEN is v10 THEN returns v10', (value) =>
		{
			...
			expect(response.version).toStrictEqual(10);
		});

	test.each(['11.282', '11.305'])
		('WHEN is v11 THEN returns v11', (value) =>
		{
			// @ts-ignore - Mocking for Foundry
			game.version = value;
			...
			expect(response.version).toStrictEqual(11);
		});
	...
	test.each(['12.282', '12.324'])
		('WHEN is v12 THEN returns v12', (value) =>
		{
			...
			expect(response.version).toStrictEqual(12);
		});

	test.each(['13.341', '13.348'])
		('WHEN is v13 THEN returns v13', (value) =>
		{
			// @ts-ignore - Mocking for Foundry
			game.version = value;
			...
			expect(response.version).toStrictEqual(13);
		});

	test.each(['14.282', '8.285'])
		('WHEN is unsupported version THEN throws Error and calls ui.notifications.error', (value) =>
		{
			// @ts-ignore - Mocking for Foundry
			game.version = value;

			const functionCall = () => Settings.getFoundryVersionStrategy();
